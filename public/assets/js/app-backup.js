...
class AmplifiApp {
    ...
    async waitForFirebaseAndInit() {
        try {
            ...
            console.warn('⚠️ Firebase initialization timeout, continuing with fallback...');
    ...
    async waitForFirebaseServices() {
        ...
        console.warn('⚠️ Firebase services not available after timeout, continuing with fallback...');
        return false;
    }
    ...
    checkFirebaseServices() {
        ...
        if (Object.values(services).every(status => status)) {
            ...
            console.warn('⚠️ Some Firebase services are missing:', services);
    ...
    async init() {
        try {
            ...
            if (typeof window.auth !== 'undefined') {
                ...
                console.warn('⚠️ Firebase auth not available, skipping auth setup');
            }
            ...
            try {
                if (typeof window.db !== 'undefined') {
                    ...
                    console.warn('⚠️ Firebase db not available, using fallback content');
                    this.loadFallbackContent();
                }
            } catch (error) {
                console.warn('⚠️ Could not load posts from Firebase, using fallback content');
                this.loadFallbackContent();
            }
...
window.addEventListener('scroll', () => {
    ...
    // Check for logout message in URL and display it
    checkForLogoutMessage() {
        ...
        if (message === "logged_out") {
            console.log("🔐 Displaying logout message");
            this.showLogoutMessage();
            
            // Open auth modal if requested
            if (auth === "login") {
                setTimeout(() => {
    ...
    showLogoutMessage() {
        ...
        messageDiv.innerHTML = `
            <div class="logout-message-content">
                ...
                <button onclick="this.parentElement.parentElement.remove()" class="logout-message-close">×</button>
            </div>
        `;
